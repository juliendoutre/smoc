...
int bum_game_register(const char *name)
{
	...
	for (i = 0; (i < PLAYER_NAME_SIZE) && name[i]; i++)
		frame[i + 1] = name[i];
...
void bum_process_player()
{
	...
	if (!event_check(xbee_get_recv_event()))
		return;
	...
	case 0x90:
	{
		...
		if (xbee_recv(&from, data, &len) && (len > 0))
		{
			...
			case BUM_GAME_PLAYERMOVE_TYPE:
				...
				if (len != BUM_GAME_PLAYERMOVE_LEN)
					return;
